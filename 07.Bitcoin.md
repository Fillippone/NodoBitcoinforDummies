...
:point right: :raising_hand: Nel mondo Linux un "demone" (*daemon* in inglese , è un programma eseguito in background, cioè senza che sia sotto il controllo diretto dell'utente, senza interfaccia e che tipicamente fornisce un servizio all'utente. Di solito i demoni hanno nomi che finiscono per "d":point left:

* Mentre siamo loggati come utente "admin", ccambiamo l'utente in “bitcoin”  
  `$ sudo su - bitcoin`
