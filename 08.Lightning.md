...
* Ancora loggati come utente Admin, create un link  simbolico al file binario `ip` che si trova nella directory `/bin/ip`, visto che sembra che LND fatichi a trovarlo in qualche caso. 
...
![Infromazioni sul Nodo LND!](images/8_06LNDGetinfo.PNG)
...
Ora il demone non mostrarà più le informazioni a schermo sulla linea di comando, ma saranno registrate nel registro di sistema. Potrete monitorare il progresso dell'avvio di LND fino a quando non sarà sinconizzato con la blockchain di testnet (circa 1.3 milios di blocchi al momento). Saranno necesarie fino a due ore, dopo di che vederete un sacco di scritte veloci (uscite con Ctrl-C).

$ sudo journalctl -f -u lnd

![Startup log di LND!](images/8_05LNDStartup.PNG)
...
![Il primo pagamento del Nodo LND!](images/8_07LNDPayment.PNG)
...
Questo è il punto di non ritorno. Fino ad ora si poteva benissimo ri-iniziare da capo. Fate esperimenti con i bitcoin di testnet, aprite e chiudete canali sulla testnet.
...
* Fate un dry run con la `lncli` e le sue varie opzioni (consultate le [Lightning API reference](http://api.lightning.community/)) :GB:
* Provate a fare qualche restart (`sudo shutdown -r now`): funziona tutto come vi sareste aspettati?

Se sì, possiamo andare avanti

---
Avanti: [Mainnet](09.mainnet.md)
