...
```
:point_right: Informazioni supplementari: [sample-lnd.conf](https://github.com/lightningnetwork/lnd/blob/master/sample-lnd.conf) nel reposaitory del progetto LND :gb: 

### Avviate LND

Come per la parte Bitcoin, perima avviamo il progrqamma manualmente con l'utente "bitcoin" per controllare che tutto funzioni correttamente.
...
* Se non volete creare un nuovo wallet, inserite la vostra `password [C]` come password del portafoglio e selezionate `n` come risposta alla domanda riguardo al seed ed inserite la password opzionale `password [D]` come password per generare il nuovo seed. Questo creerà un nuovo seed da 24 parole.
