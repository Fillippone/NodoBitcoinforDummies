...
Nella finestra riportata nell'immagine potremo avere una previw del progresso della sincronizzazione, per una view più di dettaglio, potremo aprire la finestra di debug tramite `Impostazioni` / `Finestra di debug` qui potremo tenere sotto controllo nel dettaglio le connessioni ai peer della rete Bitcoin ed il traffico generato per il download della blockchain. 
