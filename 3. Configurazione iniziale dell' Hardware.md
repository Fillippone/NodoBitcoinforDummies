...
Aprite il prompt dei comandi di Windows (`Win+R`, scrivete `cmd`, premete `Invio`, oppure cercate "Prompt dei comandi" nella barra di ricerca di Windows10 ), navigate fino alla directory bitcoin (ad esempio nel drive `D:`. Il minimo necessario che vi servirà sarà il comando '>cd' + directory per scendere in una sottodirectory rispetto a dove vi trovate, oppure 'cd..' per risalire di uno step rispetto a dove vi trovate.) e create una nuova directory `bitcoin_mainnet`. 

QUaindi calcoliamo l'hash del file che abbiamo appena scaricatoThen calculate the checksum of the already downloaded program.
```
> G:
> cd \bitcoin
