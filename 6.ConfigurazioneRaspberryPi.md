...
Assicuriamoci quindi di avere le ultime fversioni di tutti i pacchetti software installati:  
  `$ sudo apt-get install htop git curl bash-completion jq dphys-swapfile`

A questo punto vedrete il Pi scaricare tutti gli ultimi pacchetti ed aggiornare le installazioni: di solito non è necessario il riavvvio. 
