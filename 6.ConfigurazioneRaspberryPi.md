...
Il comando riportato sotto `ufw allow from 192.168.0.0/24…` ipotizza che l'indirizzio IP del vostro RaspPi sia qualcosa del tipo `192.168.0.???`, dove la parte ??? rappresenta qualsiasi numero da 0 a 255. Se l'IP del Raspberry fosse `12.34.56.78`, allora dovreste modificare la linea in `ufw allow from 12.34.56.0/24…`.
```
