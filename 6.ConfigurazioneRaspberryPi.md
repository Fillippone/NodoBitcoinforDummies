...
L'obiettivo è aumentareil grado di sicurezza del nodo ed, essendo un dispositivo collegato alla rete 24/7, diminuire la superficie di attaccco di eventuali malintenzionati. 
...
* Per prima cosa cambiamo la passwordi di default, su `1` cambiate la password con la `password [A]` che abbiamo stabilito precedentemente. .
* Aggiorniamo il software di configurazione con `8`
* Settaggi di rete, Opzione `2`: 
  * diamo al nodo un nome (like “RaspiBolt”), inoltre possiamo
  * configuriamo la connessione Wifi (solo Pi 3)
* Opaioni di Boot `3`: 
...
### Software update
é importante mantenere il sistema aggiornato con le patch di sciurezza e gli aggiornamenti delle applicazioni. Per questa tediosa operazione ci viene in soccorso l' “Advanced Packaging Tool” (apt):  
`$ sudo apt-get update`  
...
QUesta guida considera come utente principale "admin", proprio degli ambienti Linux, anzichè l'utente "pi" caratteristico dei sistemi Raspberry: cerchiamo di essere più compatibile con un ambiente Linux generico.  
...
* Copiate l'UUID che viene fornito come risutlato del comando precedente nel Notepad del Windows locale del vostro PC. 
...
* Create la directory dove montare l'hard disk e settatene il propietario corretto
...
* Create un file di testo nella nuova dicectory e poi cancellatelo per controllare che sia tutto a posto.  
...
Se questo comando vi da un errorre, allora vi sono chances che il vostro Hard Disk sia montato come “read only”. Ovvimente dovremo correggere questo errore prima di procedere. Se non ce la fate provate a ripartire da zero in quest'ultima parte e riformattare da zero l'unità.      
...
* DOme user "admin", cancelliamo il vecchio file di swap  
  `$ sudo dphys-swapfile swapoff`  
  `$ sudo dphys-swapfile uninstall`  

* Editiamo il file di configurazione e sostituiamo le vecchie entries con quelle configurate come sotto, dopo di chè salvate ed uscite.  
